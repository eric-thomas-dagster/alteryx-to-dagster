...
| **MultiRowFormula edge cases** | Pure `[Row±N:Col]` → `window_calculation`. **Forward-fill** `IF IsNull([X]) THEN [Row-1:X] ELSE [X] ENDIF` (and IIF) → `df['X'].ffill()`. **Forward-fill-with-step** `IIF(IsNull([X]), [Row-1:X] + N, [X])` (the classic "fill missing year with prev+1" idiom) → `df['X'].ffill() + (isna.groupby((~isna).cumsum()).cumcount() * N)` — vectorized, correctly handles consecutive nulls (`[2020, null, null, null, 2025]` with step=+1 → `[2020, 2021, 2022, 2023, 2025]`). **`[_CurrentField_]`** pseudo-token substituted with the actual output column before translation. Other compound IF/THEN/ELSE → `formula` with `df['Col'].shift(N)`. **Other self-referencing patterns** that don't match the above shapes are translated AND flagged with a warning — pandas `.shift(N)` returns the original value, so cascading state needs a Python loop or `.cumsum()` equivalent. Expressions with deeply nested function calls inside the row-ref still need manual review. |
