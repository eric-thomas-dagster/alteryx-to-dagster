...
| **MultiRowFormula edge cases** | Pure `[Row±N:Col]` → `window_calculation`. **Forward-fill pattern** `IF IsNull([X]) THEN [Row-1:X] ELSE [X] ENDIF` (and the IIF variant) detected and routed to `df['X'].ffill()` — correctly handles consecutive nulls (the naive `shift(1)+np.where` translation silently breaks). **`[_CurrentField_]`** pseudo-token substituted with the actual output column before translation. Compound IF/THEN/ELSE → `formula` with `df['Col'].shift(N)`. **Self-referencing expressions** like `IIF(IsNull([Year]), [Row-1:Year] + 1, [Year])` are translated AND flagged with a warning — pandas `.shift(N)` returns the original (un-computed) value, so cascading state needs a Python loop or `.cumsum()` equivalent (note surfaced in MIGRATION.md). Expressions with deeply nested function calls inside the row-ref still need manual review. |
